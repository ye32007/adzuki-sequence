...
@Service(interfaceClass = SequenceGeneratorHourRepeat.class)
@Component("sequenceGeneratorHourRepeatImpl")
public class SequenceGeneratorHourRepeatImpl extends SequenceGeneratorAble implements SequenceGeneratorHourRepeat {

	private final static Logger logger = LoggerFactory.getLogger(SequenceGeneratorHourRepeatImpl.class.getName());
	final static String baseDir = InitSystemEvnAndProp.systemEvnAndProp.get("catalina.base")+ "/cluster/hour/";
	...
	public String getNextUUID(String seqName) {
		return DateTimeUtil.getCurrentHour()+ clusterName + handler(seqName);
	}
		...
		if(map.get(seqName) == null){
			synchronized (lockObject) {
				if(null == map.get(seqName)){
					...
					//文件时间，文件内容
					if(mapped.readLength()>0 && DateTimeUtil.isToHour(mapped.readModifyTime())){
	...
	public static void main(String[] args) {
		final SequenceGeneratorHourRepeat gen = new SequenceGeneratorHourRepeatImpl();
